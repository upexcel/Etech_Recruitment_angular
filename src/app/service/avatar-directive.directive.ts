...
export class AvatarDirectiveDirective implements OnInit {
    ...
    ngOnInit() {
        if ( environment['picasaUrl'] && this.email && this.email.length && this.email.split('@')[1] === 'gmail.com') {
            this._http.get(environment['picasaUrl'] + `/${this.email}?alt=json`).subscribe((res) => {

...
describe('shortlist candidate', function() {
    ...
    // it should visit in jobprofile and see if sent mail is there or not and then click on shortlist button
    it('shortlist the candidate', function () {
     	cy.server()
        ...
        //     cy.wait('@get_emails');
        //     cy.get('#inboxTag').contains("Shortlist").click();
        // })
    })

    //check if mail found in shortlisted option of this job profile
    it('email fetched in shortlist or not', function () {
        // cy.server()
        // cy.route('PUT',data.apiUrl+`/email/fetch/**`).as('get_emails')
        // cy.get('#test').next().find('a').contains("Shortlist").click().wait('@get_emails').then(function() {
